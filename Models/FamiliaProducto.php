...
class FamiliaProducto {
    ...
    private function getQuery($id = 1) {
        $sql = "SELECT * FROM FamiliaProducto WHERE ID = :id";
        $parametros = array();
    ...
    private function setQuery($nombre) {
        ...
        $sql = "INSERT INTO FamiliaProducto (Nombre) VALUES (:nombre)";
        return $this->conn->consulta($sql, $parametros);
    }
    ...
    public function getAll($limit, $offset, $namefilter) {
        ...
        $sql = "SELECT * FROM FamiliaProducto WHERE Nombre LIKE :namefilter LIMIT :limit OFFSET :offset";
        $returnList = array();
        if ($this->conn->conectar()) {
            if ($this->conn->consulta($sql, $parametros)) {
                ...
                if (count($arr) > 0) {
                    foreach ($arr as $fila) {
                        ...
                        $familia->id = $fila["Id"];
                        $familia->nombre = $fila["Nombre"];
                        array_push($returnList, $familia);
                    }
                }
            }
        }
    ...
    public function modify($id, $nombre) {
        ...
        $sql = " UPDATE FamiliaProducto SET Nombre = :nombre WHERE Id = :id";
        if ($this->conn->conectar()) {
            if ($this->conn->consulta($sql, $parametros)) {
                return true;
            } else {
    ...
    public function remove($id) {
        $parametros = array();
        $parametros[0] = array("id", $id, "int", 11);
        $sql = "DELETE FROM FamiliaProducto WHERE Id = :id";
        if ($this->conn->conectar()) {
            if ($this->conn->consulta($sql, $parametros)) {
                return true;
            } else {
